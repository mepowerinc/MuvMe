...
@implementation AGDefaultLoadingIndicatorDesc

- (id)init {
    self = [super init];

    // reuse identifier
    self.reuseIdentifier = AG_REUSE_IDENTIFIER_LOADING;

    // width
    self.width = AGSizeMake(AG_LOADING_INDICATOR_SIZE, unitKpx);

    // height
    self.height = AGSizeMake(AG_LOADING_INDICATOR_SIZE, unitKpx);
